...
# Configuração da API
api_key = 'J23viElHlnrYGlwL75qNbhqQXkQ6DNfscfsy4UJH4Je87cNon0TqCDhjIMZ7m2t3'
api_secret = 'SyeDYucOw4CsFUzRyXkgPxS3N0MLzCKy1NGPcvvECM9qWKpNSVCdFJnYQ9F2eHIE'
...
def monte_carlo_simulation(data, num_simulations, num_days):
    ...
    for _ in range(num_simulations):
        ...
        for _ in range(num_days):
            price = prices[-1] * (1 + np.random.normal(mean, std_dev))
            prices.append(price)
...
def prepare_data_with_monte_carlo(data, look_back, scaler=None, is_train=True):
...
def predict_prices_with_monte_carlo(model, data, look_back, scaler):
...
def trading_strategy():
    ...
    predicted_price = predict_prices_with_monte_carlo(model, data, look_back, scaler)
    ...
    if predicted_price > current_price * 1.005:
        ...
        #if investment_amount / current_price > 0.0001:  # Checar se a quantidade é maior que o mínimo permitido
        buy('BTCUSDT', 0.00001)
        print("comprei")
...
# Configurar para rodar a estratégia periodicamente
#simulations = monte_carlo_simulation(data, 1000, 30)
...
#plt.ylabel('Price')
#plt.show()
#data = get_historical_data('BTCUSDT', Client.KLINE_INTERVAL_1MINUTE, (pd.Timestamp.now() - pd.Timedelta(minutes=1)).strftime('%d %b %Y %H:%M:%S'),  pd.Timestamp.now().strftime('%d %b %Y %H:%M:%S'))
#print("dados coletados")
#data = calculate_indicators(data)
...
look_back = 30
train_x, train_y, scaler = prepare_data_with_monte_carlo(train_data, look_back, is_train=True)
#test_x, test_y, _ = prepare_data_with_monte_carlo(test_data, look_back, scaler=scaler, is_train=False)
...
#predicted_price = predict_prices_with_monte_carlo(model, data, look_back, scaler)
#print(f"Preço previsto: {predicted_price}")
